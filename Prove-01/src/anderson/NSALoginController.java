...
public final class NSALoginController {
     ...
     */
    public static void hashUserPassword(User user) throws WeakPasswordException{
        ...
        try {
            if (user.getPassword().length() >= 8) {
                ...
                for (int i = 0; i < 10; i++) {
                    if (user.getPassword().contains(Integer.toString(i))) {
                        hasNum = true;
                    }
                }
            }
            if (!goodLength || !hasNum) {
                final WeakPasswordException newExc = new WeakPasswordException();
                throw newExc;
            }
        }
